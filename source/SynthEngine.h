...
public:
    ...
    void reset()
    {
        ...
        age = 0; basePan = 0.5f; releaseSamples = 0; onsetSamples = 0; postToneL = postToneR = 0.0f; warmToneL = warmToneR = 0.0f; subToneState = 0.0f; noiseDcState = 0.0f;
    }
    ...
    {
        active = true; held = true; midiNote = note; velocity = vel; age = newAge; releaseSamples = 0; onsetSamples = 0; postToneL = postToneR = 0.0f; warmToneL = warmToneR = 0.0f; subToneState = 0.0f; noiseDcState = 0.0f;
    ...
    void retarget(int note, float vel, const RuntimeParams& p, uint64_t newAge, bool retrigger)
    {
        ...
        if (retrigger)
        {
            ...
            const float seed = std::sin((float) note * 12.345f) * 43758.5453f;
            const float rnd = seed - std::floor(seed);
            oscA.setPhase((p.oscARand > 0.001f) ? (p.oscAPhase + rnd * p.oscARand) : p.oscAPhase);
            oscB.setPhase((p.oscBRand > 0.001f) ? (p.oscBPhase + rnd * p.oscBRand) : p.oscBPhase);
            sub.setPhase(0.0f);
            noise.setPhase(p.noisePhase);
        }
    }
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        const float oscALevel = p.oscALevel <= 0.0005f ? 0.0f : std::pow(p.oscALevel, 1.35f);
        const float oscBLevel = p.oscBLevel <= 0.0005f ? 0.0f : std::pow(p.oscBLevel, 1.35f);
        const float subLevel = p.subLevel <= 0.0005f ? 0.0f : 0.52f * std::pow(p.subLevel, 1.85f);
        const float noiseLevel = p.noiseLevel <= 0.0005f ? 0.0f : 0.18f * std::pow(p.noiseLevel, 2.10f);
        ...
        auto renderStack = [&](WavetableOscillator& core, std::array<WavetableOscillator, 4>& extras, float unison, float detune, float blend, float pan, bool isA)
        {
            ...
            const float centerWeight = voices == 1 ? 1.0f : juce::jmap(blend, 0.0f, 1.0f, 0.28f, 0.62f);
            const float sideWeight = voices == 1 ? 0.0f : (1.0f - centerWeight) / (float) (voices - 1);
            const float voiceNorm = 0.86f / std::sqrt((float) voices);
            float l = 0.0f, r = 0.0f;
            for (int i = 0; i < voices; ++i)
            {
                ...
                const float phaseOffset = spread * 0.07f + 0.011f * (float) i;
        ...
        subToneState += 0.18f * (subSample - subToneState);
        subSample = juce::jmap(0.42f, subSample, subToneState) * subLevel;
        ...
        noiseDcState += 0.07f * (noiseRaw - noiseDcState);
        ...
        float l = oscOutA.left * oscALevel + oscOutB.left * oscBLevel;
        float r = oscOutA.right * oscALevel + oscOutB.right * oscBLevel;
        ...
        const float sourceLevel = oscALevel + oscBLevel + subLevel + noiseLevel;
        const bool sourceMuted = sourceLevel <= 0.0005f;
        const float sourcePresence = juce::jlimit(0.0f, 1.0f, sourceLevel * 0.45f);
        ...
        const float preDrive = 1.0f + p.filterDrive * (2.5f + p.growlAmount * 1.0f) + p.toneAmount * 0.45f;
        ...
        const float distDrive = (p.distDrive + modDist * 0.18f + macroWarhorn * 0.10f + p.fmAmount * 0.08f + p.growlAmount * 0.12f + p.toneAmount * 0.05f + macroScreech * 0.08f + macroWhoop * 0.06f) * sourcePresence;
        const float foldAmt = juce::jlimit(0.0f, 1.0f, (p.motionAmount * 0.18f + p.growlAmount * 0.12f + macroWarhorn * 0.08f) * sourcePresence);
        ...
        const float onsetLen = juce::jmax(8.0f, 0.0025f * (float) sampleRate);
