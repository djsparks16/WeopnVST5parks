...
void ScopeDisplay::paint(Graphics& g)
{
    ...
    ColourGradient fill(Colour(0xff07101d), r.getTopLeft(), Colour(0xff0e1830), r.getBottomRight(), false);
    fill.addColour(0.68, Colour(0xff081322));
    g.setGradientFill(fill);
    ...
    g.drawText("phase lane · scan trail · body / growl / tone response", caption.toNearestInt(), Justification::right);
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        const float inset = 18.0f * t;
        const float y = mainView.getY() + t * mainView.getHeight();
        g.setColour(Colours::white.withAlpha(0.025f + 0.015f * (1.0f - t)));
        g.drawLine(mainView.getX() + inset, y, mainView.getRight() - inset, y, 1.0f);
    }
    for (int i = 0; i < 10; ++i)
    {
        ...
        g.drawLine(x, mainView.getY() + 10.0f, x, mainView.getBottom() - 6.0f, 1.0f);
    }

    for (int layer = 5; layer >= 0; --layer)
    {
        const float depth = (float) layer / 5.0f;
        const float scale = 1.0f - depth * 0.22f;
        const float yOffset = depth * 38.0f;
        const float xInset = depth * 22.0f;
        Path wave;
        bool started = false;
        for (size_t i = 0; i < data.size(); ++i)
        {
            ...
            const float y = mainView.getCentreY() + yOffset - data[i] * (mainView.getHeight() * 0.34f * scale);
            if (! started) { wave.startNewSubPath(x, y); started = true; }
            else wave.lineTo(x, y);
        }

        auto c = (layer % 2 == 1 ? displayB() : displayA()).withAlpha(0.08f + 0.12f * (1.0f - depth));
        g.setColour(c);
        g.strokePath(wave, PathStrokeType(6.0f - depth * 2.2f, PathStrokeType::curved, PathStrokeType::rounded));
    }
    ...
    for (size_t i = 0; i < data.size(); ++i)
    {
        const float t = (float) i / (float) (data.size() - 1);
        const float x = mainView.getX() + t * mainView.getWidth();
        const float y = mainView.getCentreY() - data[i] * mainView.getHeight() * 0.30f;
    ...
    ColourGradient area(displayA().withAlpha(0.22f), mainView.getTopLeft(), displayB().withAlpha(0.05f), mainView.getBottomLeft(), false);
    area.addColour(0.72, Colour(0x1000f0ff));
    g.setGradientFill(area);
    g.fillPath(fillPath);

    g.setColour(displayA().withAlpha(0.20f));
    g.strokePath(front, PathStrokeType(10.0f, PathStrokeType::curved, PathStrokeType::rounded));
    g.setColour(displayA());
    g.strokePath(front, PathStrokeType(2.2f, PathStrokeType::curved, PathStrokeType::rounded));

    const float beamX = mainView.getX() + 0.68f * mainView.getWidth();
    ColourGradient scan(displayA().withAlpha(0.00f), beamX - 90.0f, mainView.getY(), displayA().withAlpha(0.17f), beamX, mainView.getCentreY(), false);
    scan.addColour(0.5, displayB().withAlpha(0.11f));
    scan.addColour(1.0, displayA().withAlpha(0.00f));
    g.setGradientFill(scan);
    g.fillRoundedRectangle(Rectangle<float>(beamX - 90.0f, mainView.getY() + 10.0f, 180.0f, mainView.getHeight() - 20.0f), 14.0f);

    for (int i = 0; i < 22; ++i)
    {
        const float t = (float) i / 21.0f;
        const float x = mainView.getX() + 18.0f + t * (mainView.getWidth() - 36.0f);
        const float y = mainView.getCentreY() + std::sin(t * MathConstants<float>::twoPi * 3.0f) * 10.0f;
        const float sz = 1.5f + 2.6f * std::abs(std::sin(t * MathConstants<float>::twoPi));
        g.setColour(displayB().interpolatedWith(displayA(), t).withAlpha(0.55f));
        g.fillEllipse(x - sz * 0.5f, y - sz * 0.5f, sz, sz);
    }

    g.setColour(panel0());
    g.fillRoundedRectangle(specArea, 8.0f);
    g.setColour(displayB().withAlpha(0.16f));
    g.drawRoundedRectangle(specArea, 8.0f, 1.0f);
    for (int i = 0; i < 36; ++i)
    {
        const float t = (float) i / 35.0f;
        const float x = specArea.getX() + t * specArea.getWidth();
        const float mag = std::abs(data[(size_t) juce::jlimit(0, (int) data.size() - 1, i * 14)]);
        const float h = 8.0f + mag * (specArea.getHeight() - 18.0f);
        const float y = specArea.getBottom() - h - 6.0f;
        auto c = displayA().interpolatedWith(displayB(), t).withAlpha(0.65f);
        g.setColour(c);
        g.fillRoundedRectangle(x, y, 4.0f, h, 2.0f);
    }
    g.setColour(textLo());
    ...
    g.setColour(displayA().withAlpha(0.16f));
    ...
    for (int i = 0; i < 64; ++i)
    {
        ...
        const float y = motionArea.getCentreY() - std::sin(t * MathConstants<float>::twoPi * 1.1f) * motionArea.getHeight() * 0.25f;
...
void SpectrumDisplay::paint(Graphics& g)
{
    ...
    ColourGradient fill(displayB().withAlpha(0.24f), r.getBottomLeft(), displayA().withAlpha(0.06f), r.getTopRight(), false);
    fill.addColour(0.5, displayA().withAlpha(0.14f));
    g.setGradientFill(fill);
    g.fillPath(p);
    Path line;
    for (size_t i = 0; i < data.size(); ++i)
    ...
    g.setColour(displayA());
    g.strokePath(line, PathStrokeType(1.7f));
}
...
void LfoDisplay::paint(Graphics& g)
{
    ...
    {
        const float x = r.getX() + r.getWidth() * (float) i / (float) (data.size() - 1);
        const float y = r.getCentreY() - data[i] * r.getHeight() * 0.38f;
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), scope(p), spectrum(p), lfoShape(p), matrixView(p)
{
    ...
    logo.setText("5parks VST3 · WEAPONGRADE SYNTH", dontSendNotification);
    logo.setColour(Label::textColourId, textHi());
    logo.setFont(FontOptions(28.0f, Font::bold));
    subtitle.setText("flagship bass synth · darker chassis · larger hero waveforms · refined routing · sub / noise isolation · weapongrade bass design", dontSendNotification);
...
void FiveParksVST3AudioProcessorEditor::paint(Graphics& g)
{
    ...
    drawSection(g, matrixBox.getBounds().toFloat(), "ROUTING", modCol());

    auto kb = getLocalBounds().removeFromBottom(44).reduced(20, 0).toFloat();
    g.setColour(Colour(0xff070b14));
    ...
    {
        auto key = Rectangle<float>(kb.getX() + i * kw, kb.getY() + 3.0f, kw - 2.0f, kb.getHeight() - 6.0f);
        ColourGradient keyFill(Colour(0xff111a29), key.getTopLeft(), Colour(0xff090d15), key.getBottomLeft(), false);
        g.setGradientFill(keyFill);
        g.fillRoundedRectangle(key, 3.0f);
        auto accentKey = (i % 2 == 0 ? displayA() : displayB()).withAlpha(0.32f);
        g.setColour(accentKey);
        g.fillRoundedRectangle(Rectangle<float>(key.getX() + 1.5f, key.getY() + 1.5f, key.getWidth() - 3.0f, 2.5f), 1.8f);
...
void FiveParksVST3AudioProcessorEditor::resized()
{
    ...
    auto hero = area.removeFromTop(390);
    ...
    row1.removeFromLeft(10);
    filterBox.setBounds(row1.removeFromLeft(210));
    row1.removeFromLeft(10);
    auto stackBox = row1.removeFromLeft(280);
    subBox.setBounds(stackBox.removeFromTop(115));
    ...
    subWave.setBounds(srow.removeFromLeft(98)); srow.removeFromLeft(8); subDirect.setBounds(srow.removeFromLeft(120));
    sb.removeFromTop(6);
    subLevel.setBounds(sb.removeFromTop(140).removeFromLeft(74));

    auto nb = noiseBox.getLocalBounds().reduced(10); nb.removeFromTop(28);
    auto nrow = nb.removeFromTop(24);
    noiseType.setBounds(nrow.removeFromLeft(106)); nrow.removeFromLeft(8); noiseDirect.setBounds(nrow.removeFromLeft(120));
    nb.removeFromTop(6);
    noiseLevel.setBounds(nb.removeFromTop(140).removeFromLeft(74));
    ...
    auto mb = matrixBox.getLocalBounds().reduced(10); mb.removeFromTop(28);
    auto viewArea = mb.removeFromBottom(92);
    matrixView.setBounds(viewArea);
    for (int i = 0; i < 3; ++i)
    {
        auto row = mb.removeFromTop(38);
        ComboBox* srcs[] = { &mod1Src, &mod2Src, &mod3Src };
        ComboBox* dsts[] = { &mod1Dst, &mod2Dst, &mod3Dst };
        SerumKnob* amts[] = { &mod1Amt, &mod2Amt, &mod3Amt };
        srcs[i]->setBounds(row.removeFromLeft(86)); row.removeFromLeft(4);
        dsts[i]->setBounds(row.removeFromLeft(100)); row.removeFromLeft(4);
        amts[i]->setBounds(row.removeFromLeft(68).withHeight(94));
        mb.removeFromTop(4);
    }
    for (auto* cb : { &mod4Src, &mod4Dst, &mod5Src, &mod5Dst, &mod6Src, &mod6Dst }) cb->setBounds(0, 0, 0, 0);
    for (auto* k : { &mod4Amt, &mod5Amt, &mod6Amt }) k->setBounds(0, 0, 0, 0);
}
